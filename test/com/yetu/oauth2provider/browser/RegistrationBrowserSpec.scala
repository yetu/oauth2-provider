...
class RegistrationBrowserSpec extends BaseBrowserSpec {
  ...
  // NOTE: the route /signup was disabled for the pilot
  // so this test also needs to be disabled
  // re-enable this test upon re-enabling of the routes
  ...
  //
  //      eventually {
  //        currentUrl must be(s"http://localhost:$port/login")
  //      }
  //
